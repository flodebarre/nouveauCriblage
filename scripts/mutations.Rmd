...
```{r}
plotMut0 <- function(time, test0, test1, col, thetit, ymin = 10^-6, ymax = 1 - 10^-6, quot = TRUE, scale = "logit", addNotes = TRUE){
...
drawRec0 <- function(depDat, x, y, dxy, col = col452R, thr.p = thrp, thr.n = thrntot/7, colMaj = col452R.complement, scale = "logit", ymin = 1*10^(-3), ymax = 0.5){
  # depDat: dataset for this departement
  # x: x position of the bottom left corner
  # y: y position of the bottom left corner
  # dxy: rectangle size c(dx, dy)
...
- il y a de moins en moins de criblage dans les données publiques")), cex = cexLegend, bty = "n")
```
