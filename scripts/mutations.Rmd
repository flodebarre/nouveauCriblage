...
Links do not work because of redirection; replace by proper link

```{r}
URL_Deps <- "https://static.data.gouv.fr/resources/donnees-de-laboratoires-pour-le-depistage-indicateurs-sur-les-mutations/20210701-115629/sp-mutations-7j.csv"

URL_Regions <- "https://static.data.gouv.fr/resources/donnees-de-laboratoires-pour-le-depistage-indicateurs-sur-les-mutations/20210701-115557/sp-mutations-7j.csv"

URL_France <- "https://static.data.gouv.fr/resources/donnees-de-laboratoires-pour-le-depistage-indicateurs-sur-les-mutations/20210701-114500/sp-mutations-7j.csv"
```


```{r}
# Download files from repo
system(paste0("curl ", URL_France, " -o ../data/muts_France.csv"))
system(paste0("curl ", URL_Regions, " -o ../data/muts_Regions.csv"))
system(paste0("curl ", URL_Deps, " -o ../data/muts_Deps.csv"))
...
```{r}
plotNbCrib <- function(dat){
  par(las = 1)
  par(mar = c(4, 4, 3, 4))
  plot(as.Date(dat$date2), dat$nb_pos, ylim = c(0, 1.05*max(dat$nb_pos, na.rm = TRUE)), yaxs = "i", 
       type = "o", pch = 16, col = colTot, frame.plot = FALSE, 
       xlab = "date", ylab = "")
  
  lines(as.Date(dat$date2), dat$nb_crib, type = "o", pch = 16, col = colTotCrib)
  ...
  lines(as.Date(dat$date2), dat$nb_A0 + dat$nb_A1, type = "o", pch = 16, col = col484K)
  lines(as.Date(dat$date2), dat$nb_B0 + dat$nb_B1, type = "o", pch = 16, col = col484Q)
  lines(as.Date(dat$date2), dat$nb_C0 + dat$nb_C1, type = "o", pch = 16, col = col452R)
...
```{r}
plotMut <- function(time, test0, test1, col, thetit, ymax = 0.5){
  n <- test1 + test0
  p <- test1 / (n)
...
for(dep in unique.noNA(dat.Deps$departement)){
