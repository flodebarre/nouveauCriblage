...
```{r}
plotMut <- function(time, test0, test1, col, thetit, ymax = 1, quot = TRUE, add = FALSE, pch = 16){
  # quot: boolean to decide whether to plot the daily values (averaged over 7 days) [TRUE], or the sum over the last 7 days [FALSE]
...
if(typePlot == "IDF"){
  tmp <- dat.Regions[which(dat.Regions$nom_reg == "Ile-de-France" & dat.Regions$dateMid >= startDate), ]
  rg <- "Île-de-France"
}
if(typePlot == "France"){
  tmp <- dat.France[which(dat.France$dateMid >= startDate), ]
  rg <- "France"
}
...
plotMut(time = tmp$dateMid, test1 = tmp$nb_C0, test0 = tmp$nb_C1, col = col452R, thetit = rg)

if(compareOldNew){
  plotMut(time = tmp$dateMid, test1 = tmp$nb_C0.x, test0 = tmp$nb_C1.x, col = col452R, thetit = rg, add = TRUE, pch = pchNew)
  plotMut(time = tmp$dateMid, test1 = tmp$nb_C0.y, test0 = tmp$nb_C1.y, col = col452R, thetit = rg, add = TRUE, pch = pchOld)
}


plotMut(time = tmp$dateMid, test0 = tmp$nb_D0, test1 = tmp$nb_D1, col = colD, thetit = rg, add = TRUE, pch = pchD)

plotMut(time = tmp$dateMid, test1 = tmp$nb_A0C0, test0 = tmp$nb_A01C01 - tmp$nb_A0C0, col = colAC, thetit = rg, add = TRUE, pch = pchAC)

legend(x = as.Date("2021-11-16"), 
       y = 0.9, 
...
plotVariants <- function(time, test0, test1, pos, tot, population = 10^5, ymax = NA){
  ...
  axis(2, pos = min(dat$dateMid) + 3 - fx, lwd = 0, lwd.ticks = 1, mgp = c(0, 0.3, 0), tck = -0.015)
  axis(4, pos = max(dat$dateMid) - 3 + fx, lwd = 0, lwd.ticks = 1, mgp = c(0, 0.3, 0), tck = -0.015)
  
  xx <- seq(min(dat$dateMid) + 3, max(dat$dateMid) - 3, by = "day")
