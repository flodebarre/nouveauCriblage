...
plotTauxCrib <- function(dat){
  ...
  plot(dat$dateMid, dat$tx_crib, ylim = c(0, 100), lwd = 3, col = gray(0.3), frame.plot = FALSE, 
       xlab = "", ylab = "", yaxs = "i", type = "p", pch = 16)
  # Show final point
  i <- which(dat$dateMid == max(dat$dateMid, na.rm = TRUE))
  points(dat$dateMid[i], dat$tx_crib[i], col = "red", pch = 16)
  axis(4)
}
...
plotCribTest <- function(regname, minDate = "2021-11-15"){
  ...
  par(mar = c(3, 3, 3, 3), xpd = TRUE, mgp = c(2.5, 0.5, 0))
  lwdd <- 6
