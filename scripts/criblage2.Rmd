...
```{r}
plotMut0 <- function(time, test0, test1, col, thetit, ymin = 10^-6, ymax = 1 - 10^-6, denom = 7, scale = "logit", add = FALSE, addNotes = TRUE, pch = 16){
...
#      mtext(expression(paste("p", ", logit scale")), side = 2, las = 0, line = 2.5)
#    }
...
plotMut0(dat.France[ix, "dateMid"], dat.France[ix, "nb_C1"], dat.France[ix, "nb_C0"], col = "red", thetit = "Proportion des tests criblés avec la mutation L452R, France", ymin = 10^-4, ymax = 0.25)

plotMut0(dat.France[ix, "dateMid"], dat.France[ix, "nb_C1.dl"], dat.France[ix, "nb_C0.dl"], col = "orange", thetit = "Proportion des tests criblés avec la mutation L452R, France", ymin = 10^-4, ymax = 0.2, add = TRUE, pch = 1)

#plotReg("Île-de-France", addNotes = TRUE)

layout(1)
#plotReg("Île-de-France", addNotes = TRUE, ymax = 0.99, addLegend = TRUE)

#plotReg("Île-de-France", addNotes = TRUE, ymax = 1, ymin = 0, addLegend = TRUE, scale = "linear")


```

```{r}
plotReg <- function(reg, xmin = "2022-03-01", addNotes = FALSE, ymax = 0.05, ymin = 5*10^-4, withMut = TRUE, col = "red", scale = "logit",...){
  ...
  suffix <- ""
  plotMut0(dat$dateMid, dat[, paste0("nb_C", toString(id), suffix)], dat[, paste0("nb_C", toString(1 - id), suffix)], col = "red", thetit = "", 
           ymin = ymin, ymax = ymax, 
           addNotes = addNotes, scale = scale, ...)
  
  suffix <- ".dl"
  plotMut0(dat$dateMid, dat[, paste0("nb_C", toString(id), suffix)], dat[, paste0("nb_C", toString(1 - id), suffix)], col = "orange", thetit = "", 
           ymin = ymin, ymax = ymax, 
...
legend(x = -0.15, y = 0, legend = c("Données publiques, partagées déjà lissées", "Données dé-lissées"), 
          col = c("red", "orange"), 
          pch = c(16, 1), 
          box.col = gray(0, 0), bty = "n", cex = 1, pt.cex = 1.75, 
