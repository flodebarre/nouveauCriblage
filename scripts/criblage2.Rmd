...
```{r}
library(MetBrewer)
pal <- met.brewer("Hokusai2", 2, "discrete")
colLiss <- pal[2]
colDeliss <- pal[1]
```
...
Code: https://github.com/flodebarre/nouveauCriblage/blob/main/scripts/criblage2.Rmd", side = 1, line = 2.5, cex = 0.5, col = gray(0.5), adj = 0, family = "mono")
    ...
    legend("topleft", bty = "n", col = c(colLiss, colDeliss), pch = c(16, 1), legend = c("7-d averaged data (as provided)", "'un-smoothened' data")) 
  }
...
plotMut0(dat.France[ix, "dateMid"], dat.France[ix, "nb_C1"], dat.France[ix, "nb_C0"], col = colLiss, thetit = "Proportion of L452R+ tests, France", ymin = ym, ymax = yM)
# Add data "delisse"
...
plotReg <- function(reg, xmin = "2022-03-01", addNotes = FALSE, ymax = 0.05, ymin = 5*10^-4, withMut = TRUE, col = colLiss, scale = "logit",...){
