...
# Add data "delisse"
plotMut0(dat.France[ix, "date"], dat.France[ix, "nb_C1.dl"], dat.France[ix, "nb_C0.dl"], col = colDeliss, add = TRUE, pch = 1)
```
...
# Add data "delisse"
plotMut0(dat.France[ix, "date"], dat.France[ix, "nb_D1.dl"], dat.France[ix, "nb_D0.dl"], col = colDeliss, add = TRUE, pch = 1)
```
...
#  polygon(x = c(xpred, rev(xpred)), y = c(myinvlogit(pred2$fit - 1.96 * pred2$se.fit), rev(myinvlogit(pred2$fit + 1.96 * pred2$se.fit))), col = adjustcolor("pink", 0.4), border = NA)
  ...
  points(x = sub$date, y = sub[, n1] / (sub[, n1] + sub[, n2]))
...
axis(1, at = xpred, labels = format(xpred, "%d/%m"), las = 3, cex.axis = 0.9)
axis(2, at = logit(yvalues), labels = yvalues)
  
polygon(x = c(xpred, rev(xpred)), y = logit(c(pred$fit - 1.96 * pred$se.fit, rev(pred$fit + 1.96 * pred$se.fit))), col = adjustcolor("blue", 0.4), border = NA)

points(x = sub$date, y = logit(sub[, n1] / (sub[, n1] + sub[, n2])))
...
plotReg <- function(reg, xmin = "2022-03-01", addNotes = FALSE, ymax = 0.1, ymin = 5*10^-4, withMut = TRUE, col = colLiss, scale = "logit", percent = FALSE, ...){
...
```{r}
# Define function to plot a specific region reg
plotDep <- function(dep, xmin = "2022-03-01", addNotes = FALSE, ymax = 0.1, ymin = 5*10^-4, withMut = TRUE, col = colLiss, scale = "logit", percent = FALSE, ...){
  # withMut: whether to plot C1 / (C1 + C0) (TRUE), or C0 / (C1 + C0)
